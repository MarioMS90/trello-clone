...
export default function Workspaces() {
  ...
  const waitForChange = useAwaitChange(workspaces);
  const {
  ...
  return (
    <ul className={`mt-6 space-y-12 ${workspaces.length ? 'mb-16' : ''}`}>
      {workspaces.map(({ id, name }) => {
        const isMutating = isUpdating && updateVariables.id === id;
        const optimisticName = isMutating ? updateVariables.name : name;

        return (
          <li key={id}>
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-3">
                <WorkspaceBadge workspaceName={optimisticName} />
                <EditableText
